...
import { Heart, MapPin, Eye, Star, BadgeCheck, ImageOff } from "lucide-react";
  ...
  return (
    <div className="group h-full">
      <Link
        ...
        className="relative flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-md hover:shadow-brand-900/5"
      >
        <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
          ...
          {listing.negotiable && (
            <span className="absolute bottom-3 end-3 flex items-center gap-1 rounded-full bg-emerald-600 px-2.5 py-1 text-[11px] font-bold text-white shadow-sm">
        ...
        <div className="flex flex-1 flex-col gap-2 p-4">
          ...
          <h3 className="font-cairo line-clamp-2 min-h-12 text-[15px] font-bold leading-6 text-slate-900 transition group-hover:text-brand-700">
            {listing.title}
          </h3>

          {listing.featured && (
            <span className="flex w-fit items-center gap-1 rounded-full bg-amber-50 px-2.5 py-0.5 text-[11px] font-bold text-amber-700">
              <Star className="h-3 w-3 fill-current" /> مميز
            </span>
          )}

          <p className="font-cairo mt-auto pt-1 text-xl font-extrabold text-slate-900">
            {money(listing.price)}
          </p>
          ...
          <div className="flex items-center justify-between border-t border-slate-100 pt-3 text-xs font-semibold text-slate-500">
            <span className="flex items-center gap-1.5">
              <span className="grid h-6 w-6 place-items-center overflow-hidden rounded-full bg-slate-200 text-[10px] font-bold text-slate-600">
                {listing.user.avatar ? (
                  // eslint-disable-next-line @next/next/no-img-element
